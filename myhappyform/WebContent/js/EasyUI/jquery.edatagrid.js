	...
	function buildGrid(target){
		...
		$(target).datagrid($.extend({}, opts, {
			...
			onClickCell:function(index,field){
				...
				var row = $(this).edatagrid('getSelected');
				$('#propertygrid').propertygrid({
					 url : 'dynamicTableAction_showDynamicColumnDetailsJson?columnid='+row.colGuid,
					 showGroup:true,
					 showHeader:false
				    });
				$('#propertygriddata').propertygrid({
					 url : 'dynamicTableAction_showDynamicColumnDetailsDataJson?columnid='+row.colGuid,
					 showGroup:true,
					 showHeader:false
				    });
				...
				if (url){
					$.post(url, row, function(data){
						if (data.isError){
							...
							//opts.onError.call(target, index, data.msg);
							$(target).edatagrid('clearSelections');
							 var msg = '操作失败'+data.msg;
								common.msg('提示',msg,'remind');
							$(target).edatagrid('reload');
							return;
						}
...
//		destroyConfirmMsg: 'Are you sure you want to delete?',
		...
		onSave: function(index, row){common.msg('提示','保存成功','remind');},
		onDestroy: function(index, row){
			common.msg('提示','保存成功','remind');
		},
		onError: function(index, row){		    
			common.msg('提示','操作失败','remind');
		}
	});
})(jQuery);