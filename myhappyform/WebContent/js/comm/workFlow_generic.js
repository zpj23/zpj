...
generic_InitObjData=[],requiredArray=[] , tempData="", nextURL="" ,pluginObj;
...
var validatebox = {
	...
	validateObj:function(columnObj){
		if(columnObj.verifyinfo!=null && columnObj.verifyinfo!=""){
			var tempValidate = {colName:columnObj.colName.toUpperCase(), verifyinfo:columnObj.verifyinfo}
...
var checkbox={
	...
	initObj:function(param, columnObj, dataArray){
		param.type="checkbox";
		this.nameArray.push(columnObj.colName.toUpperCase());
...
function addLink(param,linkflag){
	...
	if(param.checked){
		$.post("processAction_addprocesslink", {islink:param.checked,linkflag:linkflag,processInstanceId:processInstanceId_});
		alert('操作成功');
	}else{
		$.post("processAction_addprocesslink", {islink:param.checked,linkflag:linkflag,processInstanceId:processInstanceId_});
		alert('取消成功');
...
var inputObj = {
	...
	init:function(plugObj, columnObj, dataArray, tagName_){
		...
		if(tagName_=="INPUT"){
			...
			if(plugObj.pluginType=="file"){
				this.input_.type="hidden";
				this.input_.name="fileid";
				this.input_['id']="fileid";
				this.input_.value = dataGuid;
				...
				nbspObj("<div id=\"myuploadify\" style=\"height:0px;width:32px;\"></div>" +
	...
	initview:function(plugObj, columnObj, dataArray, tagName_,tableGuid_){
		$.extend(inputObj, dataBinding);
		...
		if(tagName_=="INPUT"){
			this.input_ ={
			    ...
			    name: columnObj.colName.toUpperCase()+tableGuid_,
				id: columnObj.colName.toUpperCase()+tableGuid_
			}
			...
			if(plugObj.pluginType=="file"){
				...
				/*nbspObj("<div id=\"myuploadify\" style=\"height:0px;width:32px;\"></div>" +
						"<input type=\"hidden\" id=\"fileid\" name=\"fileid\" value=\""+dataGuid+"\" />" +
								"<div id=\"FileList_myuploadify\" ></div>");*/
				nbspObj("<div id=\"FileList_myuploadify"+dataGuid+"\" ></div>");
			}
...
var textareaObj = {
	init:function(plugObj, columnObj, tagName_){
		if(tagName_=="TEXTAREA"){
			...
			outerHTML_  = "<textarea id=\""+columnObj.colName.toUpperCase()+"\" name=\""+columnObj.colName.toUpperCase()+"\" style=\"width:100%;height:200px;\" ></textarea>";
		}
...
var tableObj = {
	...
	//init查看页面
	initview:function(plugObj, childResult_){
		if(plugObj.pluginTag=="table"){
			var table_tbody_ = 
				$("<table class=\"childTable tableForm\" width=\"100%\"><tbody></tbody></table>")
				.find("tbody");
			...
			$.each(childResult_[0].dataResult,function(k,v){
				...
				selectObj.init(v.plugObj, v.columnObj, v.dataArray, tagName_);
				// textarea初始化
				textareaObj.beforeInit(v.plugObj, v.columnObj, tagName_,'view');
				if(v.columnObj.isHid!=1){
					showTd_ +="<td style='border: 1px solid #95B8E7;'>"+v.columnObj.colRemark+"</td>";
				}
			});			 
...
var generic={
	// 初始化信息
	init:function(divname,id){
		$("#"+divname).append("<form id=\"tableForm"+id+"\"><table class=\"btable tableForm\" width=\"100%\"><tbody></tbody></table></form>");
	...
	// 动态表单数据展示
	ShowForm:function(param, path_, btnObj,orgGuid_,divname){
		if(param==null || param==""){
			return;
		}
		tr_=undefined;
	...
	ShowFormView:function(param, path_, btnObj,orgGuid_,divname,tableGuid_){
		//清空
		...
		if(param.layoutResult=="table"){
			...
			$.each(param.dataResult,function(k,v){
				...
				requiredVal(v.columnObj);
				// input初始化
				inputObj.initview(v.plugObj, v.columnObj, v.dataArray, tagName_,tableGuid_);
				// select初始化
				...
				// 子表初始化
				tableObj.initview(v.plugObj, param.childResult,tableGuid_);
				// 数据输出
					...
					$.each(formArray,function(k, v){
						...
						if($("#"+formObjName)[0]!=undefined){
							...
							if($("#"+formObjName)[0].nodeName=="SELECT"){
								...
								if(formObjName != 'fileid')
									
								$("#"+formObjName).val(jsonVal);
								$.each(generic_DateObjArray, function(ks,vs){
									if(formObjName.toUpperCase() == vs){
										jsonVal = jsonVal.substring(0,10);
										$("#"+formObjName).val(jsonVal);
									}
								})
							}
							if($("#"+formObjName)[0].nodeName=="TEXTAREA"||$("#"+formObjName)[0].nodeName=="DIV"){
								...
								if(!common.checkIEVersion()){
									UE.getEditor(formObjName).addListener("ready", function () {
								        	if(jsonVal==null)jsonVal="";
								            this.setContent(jsonVal);
								    });
								}else{									
									UE.getEditor(formObjName).setContent(jsonVal);
								}
							}
						}
					});
	...
	BindingDBView:function(tabData,tablename){
		//tabData.blobObjArray_ = generic_blobObjArray.join();
		var formObj_ = $('#tableForm'+tablename);
		$.ajax({
			type: "post",
			url: "queryForm",
			dataType: "html",
...
//					console.log(formArray);
					$.each(formArray,function(k, v){
						var formObjName = v.name;  
						var jsonDB = $.parseJSON(data);
						var aa = formObjName.replace(tablename,'').toUpperCase(); 
						var jsonVal = jsonDB[0][aa];
						...
						if($("#"+formObjName)[0].nodeName=="SELECT"){
							...
							var a =$("#"+formObjName)[0].parentNode;
							var req = "";
							if(jsonVal!=null&&jsonVal.indexOf(',') > -1){
...
//								console.log(jsonArray);
		...
		$.ajax({
			...
			success: function(data){
				$.each($.parseJSON(data), function(kj, vj){
					...
					childColumnArray+="<td align='center'><input type='hidden' name='"+tabData.tableName_+"_GUID' id='"+tabData.tableName_+"_GUID' value='"+$.trim(vj[tabData.tableName_+"_GUID"])+"' /><img style=\"cursor:pointer;display:none\" src=\"js/EasyUI/EasyUI1.3.6/themes/icons/search.png\" onclick=\"chooseSWRY(this)\" alt=\"选择\">&nbsp;<img style=\"cursor:pointer;\" src=\"js/EasyUI/EasyUI1.3.6/themes/icons/edit_add.png\" onclick=\"add()\" alt=\"新增\">&nbsp;<img style=\"cursor:pointer;\" src=\"js/EasyUI/EasyUI1.3.6/themes/icons/edit_remove.png\" onclick=\"del(this)\" alt=\"删除\"></td>";
			...
			success: function(data){
				$.each($.parseJSON(data), function(kj, vj){
					$.each(childObj_.dataResult, function(k,v){
						...
						var htmlObj = $(outerHTML_);
						if(htmlObj[0].tagName=="SELECT"){
						    vj[v.columnObj.colName.toUpperCase()] = htmlObj.find("option[value='"+$.trim(vj[v.columnObj.colName.toUpperCase()])+"']").text();
						}
						if(v.columnObj.isHid!=1){
							childColumnArray += $('<td>',{'style':'height:30px;border: 1px solid #CCCCCC;','align':'center'})
							.append($.trim(vj[v.columnObj.colName.toUpperCase()]))[0].outerHTML;
						}
					});
	...
	fileUploadView:function(tableName_,UUID_,path_){
		var uploadifyID = 'myuploadify'+UUID_;
		baidufile.init('view',UUID_,uploadifyID);
		$("#fileid").val(UUID_);
	...
	initObjData:function(plugObj, columnObj, dataArray){
		var jsonVal = "";
		var formObjName = columnObj.colName.toUpperCase();
		if(columnObj.isInit!=undefined){
			...
			if(dataArray[0]!=undefined && dataArray[0].orgid!=undefined){
				...
				$("#"+columnObj.colName.toUpperCase()).val(jsonVal);
			}
...
var sqleditor={
	init_:function(dataParam_, formObj_){
		...
		$.each(formArray,function(k,v){
			if(sqlMap_[v.name.toUpperCase()]){
				// 级联数组
				if(!sqlMap_[v.name.toUpperCase()].push){
					sqlMap_[v.name.toUpperCase()] = [sqlMap_[v.name.toUpperCase()]];
				}
				sqlMap_[v.name.toUpperCase()].push(v.value);
				sqlMap_[v.name.toUpperCase()] = sqlMap_[v.name.toUpperCase()].join();
			}else{
				if(v.value=="------请选择------"){
					v.value = "";
				}else{
					if(v.value!="" && new_generic_DateObjArray.indexOf(v.name.toUpperCase())!=-1){
						sqlMap_[v.name.toUpperCase()] = 'to_date(\''+v.value+'\',\'yyyy-MM-dd\')';
					}else{
						if(v.name!="result"){
							sqlMap_[v.name.toUpperCase()] = v.value;
						}
					}
				}
			}
...
function outHTML(columnObj, tagName_, plugObj_){
	...
	if (plugObj_.pluginTag == 'table') {
		...
		+ "><div style='line-height:30px;background:#E0ECFF;border: 1px solid #95B8E7;border-bottom:0px;'>&nbsp;"+columnObj.colRemark+"</div>"
		+ ""+outerHTML_+"</td>";
	}
	...
	if(tagName_=="TEXTAREA"||tagName_=="DIV"){
...
function beforOutHTML(columnObj, tagName_, plugObj_, tableGuid_){
	...
	if(tagName_=="TEXTAREA"){
		//pluginObj_.ckeditor(columnObj.colName+tableGuid_);
	}
}
...
//	},
	...
	/*UEEditor:function(param_){
		this.pluginId = param_;
		pluginObj = UE.getEditor(param_);
	...
	// 获取ckeditor信息
	getData:function(){
	    //$('#'+this.pluginId).val(pluginObj.getContent());
...
function requiredVal (columnObj){
	if(columnObj.isMust=="1"){
		...
		requiredArray.push(columnObj.colName.toUpperCase());
	}
}
						...
						$.each(formArray,function(k,v){
							...
							if(k>=1 && ++k%columnLength==0){
								...
								if(v.name==childTableName+"_GUID" && v.value==""){
									...
									if(childTableName='STABLE_SWRY'){
									mergeObj["LINK_GUID"]="LINK_GUID_DATA";
								     }
...
function chooseSWRYinfo(param){
	...
	window.parent.Dialog.OpenDialogWithReturn("选择人员",
			"/myhappyform/DialogAction_toCommonSqlDialog?type=chooseSWRYinfo",
			500,400,objArray);
}
