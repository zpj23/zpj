...
var JqueryDialog = {
	...
	OpenChild:function(dialogTitle, iframeSrc, iframeWidth, iframeHeight){
		JqueryDialog.initChild(dialogTitle, iframeSrc, iframeWidth, iframeHeight, true, true, true);
	},
	...
	init:function(dialogTitle, iframeSrc, iframeWidth, iframeHeight, isSubmitButton, isCancelButton, isDrag){
		...
		//获取客户端页面宽高
		var _client_width = $(window.parent).width();  //document.body.clientWidth;
		var _client_height = $(window.parent).height(); //document.documentElement.scrollHeight;
		
		//create shadow
		if(typeof($("#jd_shadow")[0]) == "undefined"){
			//前置
			$("body",window.parent.document).prepend("<div id='jd_shadow'>&nbsp;</div>");
			var _jd_shadow = $("#jd_shadow",window.parent.document);
		...
		$("body",window.parent.document).prepend("<div id='jd_dialog'></div>");
	
		//dialog location
		//left 边框*2 阴影5
		//top 边框*2 阴影5 header30 bottom50
		var _jd_dialog = $("#jd_dialog",window.parent.document);
		...
		var _jd_dialog_s = $("#jd_dialog_s",window.parent.document);
		...
		var _jd_dialog_m = $("#jd_dialog_m",window.parent.document);
		...
		var _jd_dialog_m_h = $("#jd_dialog_m_h",window.parent.document);
		...
		var _jd_dialog_m_b = $("#jd_dialog_m_b",window.parent.document);
		...
		var _jd_dialog_m_b_1 = $("#jd_dialog_m_b_1",window.parent.document);
		...
		//iframe
		$("#jd_dialog_m_b_2",window.parent.document).append("<iframe id='jd_iframe' name='jd_iframe' src='"+iframeSrc+"' scrolling='yes' frameborder='0' width='"+iframeWidth+"' height='"+iframeHeight+"' />");
	
		//bottom
		_jd_dialog_m.append("<div id='jd_dialog_m_t' style='background-color:"+JqueryDialog.cBottomBackgroundColor+";'></div>");
		var _jd_dialog_m_t = $("#jd_dialog_m_t",window.parent.document);
		if(isSubmitButton){
			_jd_dialog_m_t.append("<span><input id='jd_submit' class='submit_1' value='"+JqueryDialog.cSubmitText+"' type='button' onclick='JqueryDialog.Ok();' /></span>");
		}
		if(isCancelButton){
			_jd_dialog_m_t.append("<span class='jd_dialog_m_t_s'><input id='jd_cancel' class='submit_1' value='"+JqueryDialog.cCancelText+"' type='button' onclick='JqueryDialog.Close();' /></span>");
		}
		
		//register drag
		if(isDrag){
			DragAndDrop.Register(_jd_dialog.get(0), _jd_dialog_m_h.get(0));
		}
	},
    ...
    initChild:function(dialogTitle, iframeSrc, iframeWidth, iframeHeight, isSubmitButton, isCancelButton, isDrag){
		...
		_jd_dialog_m_b.append("<div id='jd_dialog_m_b_2'></div>");
		//iframe
		$("#jd_dialog_m_b_2").append("<iframe id='jd_iframe' name='jd_iframe' src='"+iframeSrc+"' scrolling='yes' frameborder='0' width='"+iframeWidth+"' height='"+iframeHeight+"' />");
	...
	Ok:function(){
		var frm = $("#jd_iframe");	
		var str = frm[0].contentWindow.Ok();
		var obj = str.split('-');
		if(obj[0]){				
			if(obj[3]=='chooseorguser'){
			JqueryDialog.submitOrgUser(obj);	
			}else if(obj[3]=='chooseorgs'){
			JqueryDialog.submitOneOrgInfo2(obj);
			}else if(obj[3]=='chooseroles'){
			JqueryDialog.submitRoles(obj);
			}else if(obj[3]=='chooseOneUser'){
			JqueryDialog.submitOneUser(obj);
			}else if(obj[3]=='modifyPwd'){
			JqueryDialog.submitModifyPwd(obj);
			}else if(obj[3]=='modifyImg'){
			JqueryDialog.submitModifyImg(obj);
			}
			else{	
			JqueryDialog.submitOneOrgInfo(obj);	
			}
			JqueryDialog.Close() ;
		}
		else{
	...
	submitOneOrgInfo: function(obj){
		var deptid = obj[1];
		var deptname = obj[2];
		var deptidsPath = obj[3];
		var deptnamesPath = obj[4];
						
		$("#parentDeptid").val(deptid); 
		$("#parentDeptname").val(deptname);
		$("#deptidsPath").val(deptidsPath);
		$("#deptnamesPath").val(deptnamesPath);
	},
	submitRoles: function(obj){
		var roleids = obj[1];
		var rolenames = obj[2];
	...
	submitOneOrgInfo2: function(obj){
		var deptid = obj[1];
		var deptname = obj[2];
	...
	submitOrgUser: function(obj){
		var userid = obj[1];
		var username = obj[2];
		
		//$(JqueryDialog.GetParentFrameItem("userids")).val(userid);
		//$(JqueryDialog.GetParentFrameItem("usernames")).val(username);	
		...
		if(tagname==''){
			...
			$("#"+tagname).val(username);
		}
	},
	submitOneUser : function(obj){
		var userid = obj[1];
		var username = obj[2];
		
		var targetid = obj[4];
		var targetname = obj[5];
						
		document.getElementById(targetid).value = userid; 
		document.getElementById(targetname).value = username;
	},
	submitModifyPwd:function(obj){
		
	},
    submitModifyImg:function(obj){
		modifyImg(obj);
	},
	...
	GetParentFrameItem:function(item){
        if(typeof(window.parent.frames["ContentBottomiframe"]) != "undefined"){
            return window.parent.frames["ContentBottomiframe"].document.getElementById(item);
        }
        else if(typeof(window.parent.frames["ifm"]) != "undefined"){
            return window.parent.frames["ifm"].document.getElementById(item);
        }
    }
};
...
var DragAndDrop = function(){
	...
	//鼠标按下
	var dragMouseDownHandler = function(evt){alert(1);
		...
		/*if(_dragObj){
			...
			//iframe遮罩
			$("#jd_dialog_m_b_1",window.parent.document).css("display", "");
	...
	//鼠标移动
	var dragMouseMoveHandler = function(evt){alert(2);
		if(_flag){
	...
	var dragMouseUpHandler = function(evt){
		if(_flag){
			...
			//取消iframe遮罩
			$("#jd_dialog_m_b_1",window.parent.document).css("display", "none");
