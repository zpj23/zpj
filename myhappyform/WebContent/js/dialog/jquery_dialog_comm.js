...
var JqueryDialog = {
	...
	Ok:function(){      alert(1);  
		...
		if(obj[0]){								
			...
			}else if(obj[3]=='chooseNodeOrgs'){
			JqueryDialog.submitNodeOrgs(obj);
			}else if(obj[3]=='chooseNodeRoles'){
			JqueryDialog.submitNodeRoles(obj);
			}else if(obj[3]=='addLine'){  
			JqueryDialog.submitAddLine(obj);
			}else if(obj[3]=='modifyLine'){  
			JqueryDialog.submitModifyLine(obj);
			}else {  
	...
	submitNodeRoles: function(obj){ 
		var roleids = obj[1];
		var rolenames = obj[2];  
		var tagid = obj[4];
		var tagname = obj[5];
		
		if(tagid==''){//默认页面id是roleids
			$("#roleids").val(roleids);
		}else{
			$("#"+tagid).val(roleids);
		}
		if(tagname==''){
			...
			$("#"+tagname).val(rolenames);
		}		
	},
	submitNodeOrgs: function(obj){
		var deptid = obj[1];
		var deptname = obj[2];
		var tagid = obj[4];
		var tagname = obj[5];   
		
		if(tagid==''){//默认页面id是parentDeptid
		...
		if(tagname==''){
			...
			$("#"+tagname).val(deptname);
		}
	},
	submitAddLine: function(obj){alert('submitAddLine');
		addLine(obj);
	},
	submitModifyLine: function(obj){
		modifyLine(obj);
	},
