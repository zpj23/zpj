...
var JqueryDialogFile = {
	...
	Open:function(dialogTitle, iframeSrc, iframeWidth, iframeHeight){
		JqueryDialogFile.init(dialogTitle, iframeSrc, iframeWidth, iframeHeight, true, true, true);
	},
	...
	Open1:function(dialogTitle, iframeSrc, iframeWidth, iframeHeight, isSubmitButton, isCancelButton, isDrag){
		JqueryDialogFile.init(dialogTitle, iframeSrc, iframeWidth, iframeHeight, isSubmitButton, isCancelButton, isDrag);
	},
	...
	init:function(dialogTitle, iframeSrc, iframeWidth, iframeHeight, isSubmitButton, isCancelButton, isDrag){
		...
		var _left = (_client_width - (iframeWidth + JqueryDialogFile.cBorderSize * 2 + 5)) / 2;
		_jd_dialog.css("left", (_left < 0 ? 0 : _left) + document.documentElement.scrollLeft + "px");
		
		var _top = (document.documentElement.clientHeight - (iframeHeight + JqueryDialogFile.cBorderSize * 2 + 30 + 50 + 5)) / 2;
		...
		_jd_dialog_s.css("width", iframeWidth + JqueryDialogFile.cBorderSize * 2 + "px");
		//iframeWidth + double border + header + bottom
		_jd_dialog_s.css("height", iframeHeight + JqueryDialogFile.cBorderSize * 2 + 30 + 50 + "px");

		//create dialog main
		_jd_dialog.append("<div id='jd_dialog_m'></div>");
		var _jd_dialog_m = $("#jd_dialog_m");
		_jd_dialog_m.css("border", JqueryDialogFile.cBorderColor + " " + JqueryDialogFile.cBorderSize + "px solid");
		_jd_dialog_m.css("width", iframeWidth + "px");
		_jd_dialog_m.css("background-color", JqueryDialogFile.cBackgroundColor);
	
		//header
		_jd_dialog_m.append("<div id='jd_dialog_m_h'></div>");
		var _jd_dialog_m_h = $("#jd_dialog_m_h");
		_jd_dialog_m_h.css("background-color", JqueryDialogFile.cHeaderBackgroundColor);
		
		//header left
		_jd_dialog_m_h.append("<span id='jd_dialog_m_h_l'>" + dialogTitle + "</span>");
		_jd_dialog_m_h.append("<span id='jd_dialog_m_h_r' title='" + JqueryDialogFile.cCloseTitle + "' onclick='JqueryDialogFile.Close();'>" + JqueryDialogFile.cCloseText + "</span>");
		...
		//bottom
		_jd_dialog_m.append("<div id='jd_dialog_m_t' style='background-color:"+JqueryDialogFile.cBottomBackgroundColor+";'></div>");
		var _jd_dialog_m_t = $("#jd_dialog_m_t");
		if(isSubmitButton){
			_jd_dialog_m_t.append("<span><input id='jd_submit' class='submit_1' value='"+JqueryDialogFile.cSubmitText+"' type='button' onclick='JqueryDialogFile.Ok();' /></span>");
		}
		if(isCancelButton){
			_jd_dialog_m_t.append("<span class='jd_dialog_m_t_s'><input id='jd_cancel' class='submit_1' value='"+JqueryDialogFile.cCancelText+"' type='button' onclick='JqueryDialogFile.Close();' /></span>");
		}
	...
	submitOneOrgInfo: function(obj){
		...
		document.getElementById("parentDeptid").value = deptid; 
		document.getElementById("parentDeptname").value = deptname;
		document.getElementById("deptidsPath").value = deptidsPath;
		document.getElementById("deptnamesPath").value = deptnamesPath;
	},
