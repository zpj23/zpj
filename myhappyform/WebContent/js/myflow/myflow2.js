(function($) {
	...
	myflow.rect = function(states, paper) {
		...
		var nameText = paper.text(currNode.attr.x + currNode.img.width + (currNode.attr.width - currNode.img.width) / 2,
				currNode.attr.y + myflow.config.lineHeight / 2, currNode.name).hide();//.attr(currNode.name);
		//text : {text:'开始'}
		var textText = paper.text(currNode.attr.x + currNode.img.width + (currNode.attr.width - currNode.img.width) / 2,
				currNode.attr.y + (currNode.attr.height - myflow.config.lineHeight) / 2 + myflow.config.lineHeight, currNode.text).hide();//.attr(currNode.text);
	...
	myflow.path = function(q, paper, fromEvent, toEvent) {
		...
		pathObj.hide();
		_pathText = paper.text(0, 0, path.text.text).attr({  //.attr(path.text)
			text : path.text.replace("{from}", fromEvent.text()).replace("{to}",
					toEvent.text())
		});
	...
	myflow.init = function(DOMObj, myflowObj) {
		...
		if (myflow.config.editable) {
			...
			$("#myflow_save").click(function() {
				...
				for (var path in myflow.config.props.props) {
					jsonStr += "\""+path +"\""+ ":\"" + myflow.config.props.props[path].value  //{\"value\":
							+ "\","//}
				}
				if (jsonStr.substring(jsonStr.length - 1, jsonStr.length) == ",") {
					jsonStr = jsonStr.substring(0, jsonStr.length - 1)
				...
				jsonStr += "}";//}}
				myflow.config.tools.save.onclick(jsonStr);
			});
		...
		if (myflowObj.restore) {
			...
			//矩形数据加载
			if (restore.states) {
				$(restore.states).each(function(index,state){
					
				
					var rect = new myflow.rect($.extend(true, {},myflow.config.tools.states[state.type],state), paper);
					//添加矩形数据到流程图显示
					rect.restore(state);
					//矩形数据临时集合
					rectArrayTemp[rect.getId()] = rect;
					//本流程图中矩形数据集合
					rectArray[rect.getId()] = rect;
				});
			}
			//路径数据加载
			if (restore.paths) {
				$(restore.paths).each(function(index,pp){
					var path = new myflow.path($.extend(true, {}, myflow.config.tools.path,pp), paper, rectArrayTemp[pp.from],rectArrayTemp[pp.to]);
					//添加路径数据到流程图显示
					path.restore(pp);
					//本流程图中路径数据集合
					pathArray[path.getId()] = path;
