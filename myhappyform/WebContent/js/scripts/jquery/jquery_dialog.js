...
var JqueryDialog = {
	...
	//配置项
	//模态窗口背景色
	"cBackgroundColor"			:	"#F2F2F2",//"#ffffff",
	...
	"cBorderSize"				:	2,
	//边框颜色
	"cBorderColor"				:	"#F2F2F2",//"#999999",

	//Header背景色
	"cHeaderBackgroundColor"	:	"#a2c6b4",//"#f0f0f0",#a2c6b4,09AEE8
	//右上角关闭显示文本
	"cCloseText"				:	"关闭",//X 关闭
	//鼠标移上去时的提示文字
	"cCloseTitle"				:	"关闭",
    ...
    //ljn add
    Open2:function(dialogTitle, iframeSrc, iframeWidth, iframeHeight,ids,names){
        JqueryDialog.init(dialogTitle, iframeSrc, iframeWidth, iframeHeight, true, true);
        
        var frm = $("#jd_iframe");


        $("body").prepend("<input type='hidden' id='stable_jd_dialog_retids' value='"+ids+"' />");
        $("body").prepend("<input type='hidden' id='stable_jd_dialog_retnames' value='"+names+"' />");
       
        //alert(document.getElementById("stable_jd_dialog_retids").value);
      
        
	},
    ChooseRiskPerson:function(dialogTitle, iframeSrc, iframeWidth, iframeHeight,ids,names,sex,mobile)
    {
         JqueryDialog.init(dialogTitle, iframeSrc, iframeWidth, iframeHeight, true, true);
        ...
        $("body").prepend("<input type='hidden' id='stable_jd_dialog_retids' value='"+ids+"' />");
        $("body").prepend("<input type='hidden' id='stable_jd_dialog_retnames' value='"+names+"' />");
        $("body").prepend("<input type='hidden' id='stable_jd_dialog_retsex' value='"+sex+"' />");
        $("body").prepend("<input type='hidden' id='stable_jd_dialog_retmobile' value='"+mobile+"' />");

    },
    AddRISKPerson:function(dialogTitle, iframeSrc, iframeWidth, iframeHeight,ids,names,sex,mobile)
    {
        JqueryDialog.init(dialogTitle, iframeSrc, iframeWidth, iframeHeight, false, true);
        var frm = $("#jd_iframe");
        $("body").prepend("<input type='hidden' id='stable_jd_dialog_retids' value='"+ids+"' />");
    ...
    /// <summary>创建跳转提示对话框</summary>
	/// <param name="dialogTitle">对话框标题</param>
	/// <param name="iframeSrc">iframe嵌入页面地址</param>
	/// <param name="iframeWidth">iframe嵌入页面宽</param>
	/// <param name="iframeHeight">iframe嵌入页面高</param>
	OpenRedirectAlert:function(dialogTitle, iframeSrc, iframeWidth, iframeHeight){
		JqueryDialog.initRedirectAlert(dialogTitle, iframeSrc, iframeWidth, iframeHeight);
    ...
    {   document.body.style.height = "100%";
        ...
        var _px_shadow = 5;var _px_top = 30;var _px_bottom = 50;
        if(!isBottom){
            _px_bottom = 0;
        }
        var _client_width = document.body.scrollWidth;
        var _client_height = document.documentElement.scrollHeight;//document.documentElement.scrollHeight;
        ...
        $("body").prepend("<div id='jd_dialog' style='display:none;'></div>");
       ...
       // var _top = (document.documentElement.scrollHeight - (iframeHeight + JqueryDialog.cBorderSize * 2 + _px_top + _px_bottom + JqueryDialog.cShdowDown)) / 2;
       /*此处手动修改*/
      var temptop=document.documentElement.scrollTop || document.body.scrollTop;
      var _top=temptop+80;
       _jd_dialog.css("top", (_top < 0 ? 0 : _top)  + "px");
      //  _jd_dialog.css("top", _top + "px");
        _jd_dialog.append("<div id='jd_dialog_s'>&nbsp;</div>");
            
        var _jd_dialog_s = $("#jd_dialog_s");
        _jd_dialog_s.css("width", iframeWidth+2 + JqueryDialog.cBorderSize * 2 + "px");
        _jd_dialog_s.css("height", iframeHeight+2 + JqueryDialog.cBorderSize * 2 + _px_top + _px_bottom + "px");
       ...
       // _jd_dialog_m_h.css("color", "#ffffff");
       // _jd_dialog_m_h.css("font-size", "12px");
        ...
        $("#jd_dialog_m_b_2").append("<iframe id='jd_iframe' src='"+iframeSrc+"' scrolling='yes' frameborder='0' width='"+iframeWidth+"' height='"+iframeHeight+"' />");
        ...
        if(isBottom){
            ...
            _jd_dialog_m_t.append("<span><input id='jd_submit' class='BigButton' value='"+JqueryDialog.cSubmitText+"' type='button' onclick='JqueryDialog.Ok();' /></span>");
            _jd_dialog_m_t.append("<span class='jd_dialog_m_t_s'><input id='jd_cancel' class='BigButton' value='"+JqueryDialog.cCancelText+"' type='button' onclick='JqueryDialog.Close();' /></span>");
        }
    ...
    initRedirectAlert:function(dialogTitle, iframeSrc, iframeWidth, iframeHeight)
    {
        var _px_shadow = 5;var _px_top = 30;var _px_bottom = 50;
//        if(!isBottom){
//            _px_bottom = 0;
//        }
        ...
        var _client_height = document.documentElement.clientHeight;//document.documentElement.scrollHeight;
    ...
    SubmitCompleted:function(alertMsg, isCloseDialog, isRefreshPage)
    {
        ...
        if(isCloseDialog){JqueryDialog.Close();}
        if(isRefreshPage){window.location.href = window.location.href;}
    },
            ...
            var _mouseCurrentLocation = {x : evt.screenX,y : evt.screenY};
                ...
                if(evt.preventDefault)
                {
                    ...
                    var dragMouseUpHandler = function(evt)
                        {
                            if(_flag){
                                evt = evt || window.event;$("#jd_dialog_m_b_1").css("display", "none");
                                cleanMouseHandlers();
                                _flag = false;
                            }
                        };
                        ...
                        return {
                            Register : function(dragObj, controlObj){
                                _dragObj = dragObj;_controlObj = controlObj;
