...
function addBookmark() {
		...
		document.all.WebOffice1.SetFieldValue("test", "加入书签^_^", "::ADDMARK::");	
		

}
function taohong(){
	alert("给名为test的书签添加套红信息");	
	document.all.WebOffice1.SetFieldValue("test", "", "::ADDMARK::");
	//document.all.WebOffice1.SetFieldValue("test", "F:\\taohong.doc", "::FILE::");
	document.all.WebOffice1.SetFieldValue("test", "http://127.0.0.1:8089/Core/js/weboffice/doc/taohong.doc", "::FILE::");
}
function gaizhang(){
	document.all.WebOffice1.SetFieldValue("test", "", "::ADDMARK::");
	var zhangurl = "http://127.0.0.1:8089/Core/js/weboffice/img/zhang.gif"; //"http://127.0.0.1:8089/Core/weboffice/img/3.jpg"
...
function SaveDoc(id,docType) {
	try{
		...
		returnValue = webObj.HttpPost("/uploadfileAction_uploadFile");	// 判断上传是否成功
...
function AddSeal_onclick()
{
	try{
		var webObj=document.getElementById("WebOffice1");
   		 //通过Document->application->CommandBars 获取到菜单对象
  		var vObj = webObj.GetDocumentObject().Application.CommandBars("电子印章");alert(vObj);
