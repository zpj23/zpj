...
public class SetCharacterFilter implements Filter{
	...
	public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse,
		...
		// 获取session中用户信息
		SysUserinfo user = (SysUserinfo) req.getSession().getAttribute("iuserinfo");		
		if (user != null) {
			isLogin = true;
		}
		if(isLogin){
		...
		}else if(str_href.indexOf("/loginAction_checkLogin")>-1){//第一次登录
			chain.doFilter(servletRequest, servletResponse);
		}else if(str_href.indexOf("/loginAction_uploadie")>-1){//第一次登录
			chain.doFilter(servletRequest, servletResponse);
