...
public class OfficeServiceImpl implements OfficeService {
    ...
    private iFileUpLoad2000 FileObj;
    ...
    //==============================↓Bolb字段处理代码【开始】↓==============================
    //向数据库写文档数据内容
    public void PutAtBlob(Blob blob, int vSize) throws IOException {
      try {
        OutputStream outstream = blob.setBinaryStream(vSize);
    ...
    private boolean LoadFile() {
      ...
      String Sql = "SELECT FileBody,FileSize FROM WEBOFFICE_Document_File WHERE RecordID='" + mRecordID + "'";
      try {
    ...
    private boolean SaveFile() {
      boolean mResult = false;
      int iFileId = -1;
      String Sql = "SELECT * FROM WEBOFFICE_Document_File WHERE RecordID='" + mRecordID + "'";
      try {
        if (DbaObj.OpenConnection()) {
          try {
            ResultSet result = DbaObj.ExecuteQuery(Sql);
            if (result.next()) {
              //Sql = "update WEBOFFICE_Document_File set FileID=?,RecordID=?,FileName=?,FileType=?,FileSize=?,FileDate=?,FileBody=EMPTY_BLOB(),FilePath=?,UserName=?,Descript=? WHERE RecordID='" + mRecordID + "'";
            	Sql = "update WEBOFFICE_Document_File set FileID=?,RecordID=?,FileName=?,FileType=?,FileSize=?,FileDate=?,FileBody=?,FilePath=?,UserName=?,Descript=? WHERE RecordID='" + mRecordID + "'";
              iFileId = result.getInt("FileId");
            }
            else {
              //Sql = "insert into WEBOFFICE_Document_File (FileID,RecordID,FileName,FileType,FileSize,FileDate,FileBody,FilePath,UserName,Descript) values (?,?,?,?,?,?,EMPTY_BLOB(),?,?,? )";
              Sql = "insert into WEBOFFICE_Document_File (FileID,RecordID,FileName,FileType,FileSize,FileDate,FileBody,FilePath,UserName,Descript) values (?,?,?,?,?,?,?,?,?,? )";
              iFileId = DbaObj.GetMaxID("WEBOFFICE_Document_File", "FileId");
            }
            result.close();
          }
          catch (SQLException e) {
...
//            ResultSet update =  stmt.executeQuery("select FileBody from WEBOFFICE_Document_File where Fileid=" + String.valueOf(iFileId) + " for update");
//            if (update.next()) {
//              try {
//                PutAtBlob(update.getBlob("FileBody"), mFileSize);
//              }
//              catch (IOException e) {
//                System.out.println(e.toString());
//                mResult = false;
...
//            DbaObj.Conn.commit();
    ...
    private boolean LoadTemplate() {
      boolean mResult = false;
      String Sql = "SELECT FileBody,FileSize FROM WEBOFFICE_Template_File WHERE RecordID='" + mTemplate + "'";
      try {
        if (DbaObj.OpenConnection()) {
          try {
            ResultSet result = DbaObj.ExecuteQuery(Sql);
    ...
    private boolean SaveTemplate() {
      boolean mResult = false;
      String Sql = "SELECT * FROM WEBOFFICE_Template_File WHERE RecordID='" + mTemplate + "'";
      try {
        if (DbaObj.OpenConnection()) {
          try {
            ResultSet result = DbaObj.ExecuteQuery(Sql);
            if (result.next()) {
              //Sql = "update WEBOFFICE_Template_File set TemplateID=?,RecordID=?,FileName=?,FileType=?,FileSize=?,FileDate=?,FileBody=EMPTY_BLOB(),FilePath=?,UserName=?,Descript=? WHERE RecordID='" + mTemplate + "'";
              Sql = "update WEBOFFICE_Template_File set TemplateID=?,RecordID=?,FileName=?,FileType=?,FileSize=?,FileDate=?,FileBody=?,FilePath=?,UserName=?,Descript=? WHERE RecordID='" + mTemplate + "'";
              mTemplateId = result.getInt("TemplateId");
            }
            else {
              //Sql = "insert into WEBOFFICE_Template_File (TemplateID,RecordID,FileName,FileType,FileSize,FileDate,FileBody,FilePath,UserName,Descript) values (?,?,?,?,?,?,EMPTY_BLOB(),?,?,? )";
              Sql = "insert into WEBOFFICE_Template_File (TemplateID,RecordID,FileName,FileType,FileSize,FileDate,FileBody,FilePath,UserName,Descript) values (?,?,?,?,?,?,?,?,?,? )";
              mTemplateId = DbaObj.GetMaxID("WEBOFFICE_Template_File", "TemplateId");
            }
            result.close();
          }
          catch (SQLException e) {
          ...
          try {
            ...
            DbaObj.Conn.setAutoCommit(false);
            stmt = DbaObj.Conn.createStatement();
            ResultSet update =  stmt.executeQuery("select FileBody from WEBOFFICE_Template_File where TEMPLATEID=" + String.valueOf(mTemplateId) + " for update");
            if (update.next()) {
              try {
    ...
    private boolean ListVersion() {
      boolean mResult = false;
      String mDesc = "";
      String Sql = "SELECT FileID,FileDate,UserName,Descript FROM WEBOFFICE_Version_File WHERE RecordID='" + mRecordID + "'";
      mFileID = "\r\n";
      mDateTime = "保存时间\r\n";
      mUserName = "用户名\r\n";
      mDescript = "版本说明\r\n";
    ...
    private boolean LoadVersion(String mFileID) {
      boolean mResult = false;
      String Sql = "SELECT FileBody,FileSize FROM WEBOFFICE_Version_File WHERE RecordID='" + mRecordID + "' and  FileID=" + mFileID;
      try {
        if (DbaObj.OpenConnection()) {
          try {
            ResultSet result = DbaObj.ExecuteQuery(Sql);
    ...
    private boolean SaveVersion() {
      boolean mResult = false;
      int iFileId = -1;
      //String Sql = "insert into WEBOFFICE_Version_File (FileID,RecordID,FileName,FileType,FileSize,FileDate,FileBody,FilePath,UserName,Descript) values (?,?,?,?,?,?,EMPTY_BLOB(),?,?,? )";
      String Sql = "insert into WEBOFFICE_Version_File (FileID,RecordID,FileName,FileType,FileSize,FileDate,FileBody,FilePath,UserName,Descript) values (?,?,?,?,?,?,null,?,?,? )";
      ...
      try {
        if (DbaObj.OpenConnection()) {
          ...
          try {
            ...
            DbaObj.Conn.setAutoCommit(false);
            stmt = DbaObj.Conn.createStatement();
            ResultSet update =  stmt.executeQuery("select FileBody from WEBOFFICE_Version_File where Fileid=" + String.valueOf(iFileId) + " for update");
            if (update.next()) {
              try {
                GetAtBlob(update.getBlob("FileBody"), mFileSize);
              }
              catch (IOException e) {
                System.out.println(e.toString());
                mResult = false;
    ...
    private boolean ListBookmarks() {
      boolean mResult = false;
      String Sql = "SELECT * FROM WEBOFFICE_Bookmarks ";
      mBookmark = "";
      mDescript = "";
      try {
        if (DbaObj.OpenConnection()) {
    ...
    private boolean LoadBookMarks() {
      boolean mResult = false;
      String Sql = " select b.BookMarkName,b.BookMarkText from WEBOFFICE_Template_BookMarks a,WEBOFFICE_BookMarks b where a.BookMarkname=b.BookMarkName and a.RecordID='" + mTemplate + "'";
      try {
        if (DbaObj.OpenConnection()) {
          try {
            ResultSet result = DbaObj.ExecuteQuery(Sql);
    ...
    private boolean SaveBookMarks() {
      ...
      try {
        if (DbaObj.OpenConnection()) {
          try {
            java.sql.PreparedStatement prestmt = null;
            String Sql = "DELETE FROM WEBOFFICE_Template_BookMarks Where RecordID='" + mTemplate + "'";
            prestmt = DbaObj.Conn.prepareStatement(Sql);
            DbaObj.Conn.setAutoCommit(true);
            prestmt.execute();
            DbaObj.Conn.commit();
            ...
            for (mIndex = 7; mIndex <= MsgObj.GetFieldCount() - 1; mIndex++) {
              ...
              mBookMarkId = DbaObj.GetMaxID("WEBOFFICE_Template_BookMarks", "BookMarkId");
              Sql = "insert into WEBOFFICE_Template_BookMarks (BookMarkId,RecordId,BookMarkName) values ('" + String.valueOf(mBookMarkId) + "','" + mTemplate + "','" + mBookMarkName + "')";
    ...
    private boolean LoadMarkList() {
      String Sql = "SELECT MarkName FROM WEBOFFICE_Signature";
      mMarkList = "";
      boolean mResult = false;
      try {
        if (DbaObj.OpenConnection()) {
    ...
    private boolean LoadMarkImage(String vMarkName, String vPassWord) {
      String Sql = "SELECT MarkBody,MarkType,MarkSize FROM WEBOFFICE_Signature WHERE MarkName='" + vMarkName + "' and PassWord='" + vPassWord + "'";
      boolean mResult = false;
      try {
        if (DbaObj.OpenConnection()) {
          try {
            ...
            if (result.next()) {
              try {
                ...
                GetAtBlob(( result).getBlob("MarkBody"), mFileSize);
    ...
    private boolean SaveSignature() {
      boolean mResult = false;
      int iSignatureID;
      String Sql = "insert into WEBOFFICE_Document_Signature (SignatureID,RecordID,MarkName,UserName,DateTime,HostName,MarkGuid) values (?,?,?,?,to_date('" + mDateTime + "','yyyy-mm-dd hh24:mi:ss'),?,? ) ";
      iSignatureID = DbaObj.GetMaxID("WEBOFFICE_Document_Signature", "SignatureID");
      if (DbaObj.OpenConnection()) {
        java.sql.PreparedStatement prestmt = null;
        try {
          prestmt = DbaObj.Conn.prepareStatement(Sql);
    ...
    private boolean LoadSignature() {
      boolean mResult = false;
      String Sql = "SELECT MarkName,UserName,DateTime,HostName,MarkGuid FROM WEBOFFICE_Document_Signature WHERE RecordID='" + mRecordID + "'";
      mMarkName = "印章名称\r\n";
      mUserName = "签名人\r\n";
      mDateTime = "签章时间\r\n";
      mHostName = "客户端IP\r\n";
      ...
      try {
        if (request.getMethod().equalsIgnoreCase("POST")) {
          ...
          if (MsgObj.GetMsgByName("DBSTEP").equalsIgnoreCase("DBSTEP")) {         //判断是否是合法的信息包，或者数据包信息是否完整
            ...
            if (mOption.equalsIgnoreCase("LOADFILE")) {                           //下面的代码为打开服务器数据库里的文件
              ...
              if (MsgObj.MsgFileLoad(mFilePath)) {                                //调入文档内容
            ...
            else if (mOption.equalsIgnoreCase("SIGNATRUELIST")) {                    //下面的代码为创建印章列表
                MsgObj.MsgTextClear();                                            //清除SetMsgByName设置的值
                if (LoadMarkList()) {                                            //调入印章列表
                ...
                MsgObj.MsgTextClear();                                            //清除SetMsgByName设置的值
                if (LoadMarkImage(mMarkName, mPassword)) {                       //调入印章
